...
export default function PromptTimeline({ history }: PromptTimelineProps) {
  return (
    <div className="rounded-lg bg-secondary/80 border border-border/50 p-4" role="region" aria-label="提示词进化时间线">
      ...
      {history.length === 0 ? (
        ...
        <div className="relative pl-6">
          {/* Timeline line */}
          <div className="absolute left-2.5 top-0 bottom-0 w-px bg-primary/40" aria-hidden="true" />
...
function TimelineEntry({ entry }: { entry: PromptHistoryEntry }) {
  ...
  return (
    <div className="relative pb-4" role="article" aria-label={`版本 ${entry.version} 到 ${entry.version + 1} 的变更`}>
      {/* Node dot */}
      <div className="absolute -left-3.5 h-3 w-3 rounded-full bg-card border-2 border-primary" aria-hidden="true" />
        ...
        {expanded && (
          <div className="mt-2 space-y-2">
            <div className="rounded bg-card/60 p-2">
              <div className="text-xs text-destructive mb-1">旧提示词 (前100字):</div>
              <div className="text-xs text-muted-foreground max-h-24 overflow-y-auto">
                {entry.old_prompt.slice(0, 200)}...
              </div>
            </div>
            <div className="rounded bg-card/60 p-2">
              <div className="text-xs text-success mb-1">新提示词 (前100字):</div>
              <div className="text-xs text-muted-foreground max-h-24 overflow-y-auto">
