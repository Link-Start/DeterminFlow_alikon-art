  ...
  return (
    <div role="presentation" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose} onKeyDown={(e) => { if (e.key === "Escape") onClose(); }}>
      <div
        ...
        aria-modal="true"
        aria-label="管理工具分组"
        className="bg-secondary border border-border/50 rounded-xl p-6 w-[520px] max-h-[80vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-foreground">管理工具分组</h2>
          <button ref={closeBtnRef} type="button" onClick={onClose} aria-label="关闭" className="p-1 min-w-[44px] min-h-[44px] flex items-center justify-center text-muted-foreground hover:text-foreground cursor-pointer focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none">
        ...
        {/* Error */}
        {error && (
          <div role="alert" className="mb-3 px-3 py-2 rounded-md bg-destructive/10 border border-destructive/30 text-xs text-destructive">
            {error}
          </div>
        )}
            ...
            return (
              <div key={group.id} className="flex items-center justify-between p-3 bg-secondary/60 rounded-lg border border-border/30">
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 bg-card/60 border border-primary/50 rounded px-2 py-1 text-xs text-foreground outline-none"
                        autoFocus
                        onKeyDown={(e) => {
                      ...
                      <button
                        ...
                        aria-label="确认重命名"
                        className="p-1 min-w-[44px] min-h-[44px] flex items-center justify-center text-success hover:text-success/80 cursor-pointer disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-success/30 focus-visible:outline-none"
                      >
                      ...
                      <button
                        ...
                        onClick={cancelEdit}
                        aria-label="取消重命名"
                        className="p-1 min-w-[44px] min-h-[44px] flex items-center justify-center text-muted-foreground hover:text-foreground cursor-pointer focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none"
                      >
                    ...
                    <>
                      <div className="text-xs font-medium text-foreground">{group.name}</div>
                ...
                <div className="flex items-center gap-2 ml-3">
                  <Badge
                    variant="outline"
                    className={`text-xs ${toolCount > 0 ? "text-info border-info/30" : "text-muted-foreground border-muted-foreground/30"}`}
                  >
                    {toolCount} 个工具
                  </Badge>
                  {!isEditing && (
                    <button
                      ...
                      className="p-1 min-w-[44px] min-h-[44px] flex items-center justify-center text-muted-foreground hover:text-primary transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none"
                    >
                  ...
                  <button
                    ...
                    className={`p-1 min-w-[44px] min-h-[44px] flex items-center justify-center transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-destructive/30 focus-visible:outline-none ${
                      toolCount > 0
        ...
        <div className="border-t border-border/30 pt-4">
          <h3 className="text-xs font-medium text-foreground mb-3 flex items-center gap-1">
            <Plus size={12} className="text-primary" aria-hidden="true" />
            新建分组
          </h3>
          <div className="space-y-2.5">
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                aria-label="添加新分组"
                className="flex items-center gap-1 px-3 py-1.5 min-h-[44px] text-xs rounded-md bg-primary/15 text-primary hover:bg-primary/25 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none"
              >
