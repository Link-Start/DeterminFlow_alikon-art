...
export default function StatsCards({ status }: StatsCardsProps) {
  const cards = [
    ...
    },
    {
      ...
      icon: Wifi,
      iconColor: status.mcp_connected ? "text-success" : "text-destructive",
      borderColor: status.mcp_connected ? "border-success/20" : "border-destructive/20",
    },
  ];
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4" role="region" aria-label="系统状态统计">
      {cards.map((card) => {
        ...
        return (
          <div
            key={card.label}
            className={`p-4 rounded-lg bg-secondary/80 border ${card.borderColor} hover:scale-[1.02] transition-transform`}
            role="article"
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {card.pulse && (
                <span className="w-2 h-2 rounded-full bg-success status-running" aria-hidden="true" />
              )}
            </div>
